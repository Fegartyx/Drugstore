...
function calculateChange(itemCarts) {
    let price = 0;
    for (const object of itemCarts) {
        price = object.price * object.qty;
    }
    console.log(price);
    // Ambil nilai dari input
    const payTotal =
        parseFloat(document.getElementsByName("total_bayar")[0].value) || 0;
    const itemTotal =
        parseFloat(document.getElementById("item_total").value) || 0;

    // Hitung kembalian
    const change = payTotal - itemTotal;

    // Tampilkan kembalian
    document.getElementById("return_total").value = change;
}
